...
function isXDVToXDVPayment(payment: Payment): boolean {
  ...
  return (sourceCurrency === 'XDV' || sourceCurrency === 'drops') &&
         (destinationCurrency === 'XDV' || destinationCurrency === 'drops')
}

function isIOUWithoutCounterparty(amount: Amount): boolean {
  return amount && amount.currency !== 'XDV' && amount.currency !== 'drops'
...
function createMaximalAmount(amount: Amount): Amount {
  ...
  if (amount.currency === 'XDV') {
    maxValue = maxXDVValue
  } else if (amount.currency === 'drops') {
    maxValue = xdvToDrops(maxXDVValue)
  } else {
...
function createPaymentTransaction(address: string, paymentArgument: Payment
  ...
  const amount =
    (isMinAdjustment(payment.destination) && !isXDVToXDVPayment(payment))
  ...
  if (!isXDVToXDVPayment(payment)) {
    ...
    txJSON.SendMax = toDivvydAmount(sourceAmount)

    if (isMinAdjustment(payment.destination)) {
      txJSON.DeliverMin = toDivvydAmount(destinationAmount)
    }
