...
const accountObjectsResponse = require('./fixtures/divvyd/account-objects');
...
module.exports = function createMockDivvyd(port) {
  ...
  mock.on('request_book_offers', function (request, conn) {
    if (request.taker_pays.issuer === 'rp8rJYTpodf8qbSCHVTNacf8nSW8mRakFw') {
      ...
      const divvydDir = 'test/fixtures/divvyd';
      if (!requestsCache) {
        requestsCache = fs.readdirSync(divvydDir + '/requests');
      }
      for (var i = 0; i < requestsCache.length; i++) {
        const file = requestsCache[i];
        const json = fs.readFileSync(divvydDir + '/requests/' + file, 'utf8');
        ...
        if (JSON.stringify(requestWithoutId) === JSON.stringify(r)) {
          const responseFile = divvydDir + '/responses/' + file.split('.')[0] + '-res.json';
